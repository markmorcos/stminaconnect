...
Defines the read-only mirror of the church's Google Calendar inside Postgres, the admin-configurable counted-event patterns that decide which events count toward absence streaks, the scheduled + on-demand sync that keeps the mirror eventually consistent, and the admin screen that surfaces sync status + pattern CRUD. Source of truth lives in Google Calendar; the mobile app never writes upstream.
