...
Defines the admin overview screen that summarizes engagement at a glance: total members, recent activity, attendance trend, who's at risk, newcomer funnel, and member distribution by region. All sections are read-only aggregations served by per-section `dashboard_*` RPCs so they fetch in parallel and a slow section never blocks the others. Also covers the admin-only servant lifecycle (invite, promote/demote, deactivate/reactivate) reached from Settings → Admin → Servants.
