...
Defines the navigation shell shared by every authenticated screen: a four-tab bottom bar (Home / Persons / Follow-ups / Settings), a per-tab header carrying the notifications bell + a dev-only overflow menu, a role-aware Home tab that renders the admin dashboard or the servant launcher in place at `/`, and a sectioned Settings landing that consolidates account, language, about, sign-out and the admin sub-pages. The shell exists so feature changes don't reinvent navigation primitives, and so production builds present no developer affordances by default.
