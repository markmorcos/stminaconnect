...
- **ADDED** `MockNotificationService` implementation:
  ...
  - On insert: shows in-app Snackbar/banner ("You have a new alert"), optionally fires a local notification via `expo-notifications`.
...
- **Affected code**: new `src/services/notifications/*`, new `app/(app)/notifications.tsx`, `app/_layout.tsx` (provider mount), `supabase/migrations/007_notifications.sql`. New banner component.
- **Breaking changes**: none.
- **Migration needs**: one migration.
- **Expo Go compatible**: yes — the mock dispatcher uses Supabase Realtime + Paper Banner + optional `expo-notifications` local notifications, all of which work in Expo Go.
