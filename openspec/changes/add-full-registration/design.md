...
8. **Soft-delete UI**: a Paper Button with destructive intent ("Remove member") below the Edit form's Save button, admin-only, opens a confirmation dialog with a typed-confirmation pattern (admin must type the person's name to enable the Confirm button). This is the *general churn* path (PII scrub, attendance preserved); GDPR Article 17 hard-erasure is a distinct admin path introduced in `add-gdpr-compliance` (Admin Compliance screen) and must not be conflated with this soft-delete UI.
