...
9. **Magic-link deep-link activation** (resolves the deferred half of `add-servant-auth`'s decision 2): the dev client registers `stminaconnect://`, so the magic-link URL Supabase already includes in every email becomes tappable end-to-end here for the first time. `add-servant-auth` shipped with the OTP-code path as the canonical sign-in alternate (Expo Go's local GoTrue silently rejects `exp://` redirects and falls back to `site_url`); this phase verifies that `app/(auth)/callback.tsx` correctly exchanges the code for a session when the OS opens the app via `stminaconnect://auth/callback?code=…`. The OTP-code path remains supported as a fallback for users who can't tap the link (e.g. opened the email on a different device). No code changes are expected — the route is already wired; we're just exercising it on a build that supports the scheme.
